function Pob = initialize_variables(N, M, V, min_range, max_range)
...
%%% CREAR POBLACION INICIAL  (mejora eliminando bucle interno)

Pob=repmat(min+(max-min),N,1).*rand(N,3);

%% **************el siguiente bucle también se podrá quitar, cambiando la función de evaluación
...
% % % % % % %     end
    ...
    Pob(i,V + 1: K) = evaluate_objective(Pob(i,:), M, V);
end
